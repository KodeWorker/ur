...
int main() {
  ...
  while (!WindowShouldClose() && !shouldQuit) {
    ...
    if (currentScreen == MENU) {
      DrawText("MAIN MENU", screenWidth / 2 - MeasureText("MAIN MENU", 40) / 2,
               60, 40, DARKGRAY);

      // Draw Offline Button
      DrawRectangleRec(
          btnOffline,
          CheckCollisionPointRec(mousePoint, btnOffline) ? LIGHTGRAY : GRAY);
      ...
      DrawRectangleRec(
          btnOptions,
          CheckCollisionPointRec(mousePoint, btnOptions) ? LIGHTGRAY : GRAY);
      DrawText("OPTIONS", btnOptions.x + 50, btnOptions.y + 15, 20, BLACK);

      // Draw Quit Button
      DrawRectangleRec(
          btnQuit, CheckCollisionPointRec(mousePoint, btnQuit) ? RED : MAROON);
      ...
      DrawText("CONNECT TO SERVER",
               screenWidth / 2 - MeasureText("CONNECT TO SERVER", 40) / 2, 40,
               40, DARKBLUE);
      ...
      DrawText("Username:", usernameBox.x - 110, usernameBox.y + 10, 20,
               DARKGRAY);
      ...
      DrawText("Password:", passwordBox.x - 110, passwordBox.y + 10, 20,
               DARKGRAY);

      // Display password as asterisks
      std::string maskedPassword(passwordInput.length(), '*');
      DrawText(maskedPassword.c_str(), passwordBox.x + 5, passwordBox.y + 10,
               20, BLACK);

      // Draw Connect button
      DrawRectangleRec(
          btnConnect,
          CheckCollisionPointRec(mousePoint, btnConnect) ? DARKGREEN : GREEN);
      DrawText("CONNECT", btnConnect.x + 50, btnConnect.y + 15, 20, WHITE);

      // Draw Back button
      DrawRectangleRec(btnBack, CheckCollisionPointRec(mousePoint, btnBack)
                                    ? LIGHTGRAY
                                    : GRAY);
